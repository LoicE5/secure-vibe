...
- Mount the host `~/.claude-code-router` **read-only** (mirrored writable in-container); scaffold a minimal starter `config.json` when none exists, and pin `HOST` to `127.0.0.1` when no `APIKEY` is set so the router is never bound wide (the interactive Claude Code session is preserved — `NON_INTERACTIVE_MODE` is left untouched)
- Pin CCR's `CLAUDE_PATH` to the in-container `claude` wrapper so the routed Claude Code always inherits `--dangerously-skip-permissions` + the sandbox prompt, independent of PATH ordering
