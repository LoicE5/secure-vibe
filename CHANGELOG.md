...
- Surface gateway startup failures instead of hanging: the entrypoint polls `/health`, fails fast if the child dies, and special-cases CCR 3.x's refusal to start when no provider lists a model. A failure leaves you in a usable shell with a pointer to `~/.ccr-serve.log` rather than killing the container
...
- Add `scripts/test-ccr.sh`, an end-to-end suite covering the whole provider — image contents, runtime env, gateway health, a real round trip and tool call via `claude -p`, least-privilege forwarding, host-config immutability, and validation of provider model ids against OpenRouter's live list
