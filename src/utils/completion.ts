...
function findValueFlag(name: string | undefined): ValueFlag | undefined {
  ...
  return FLAGS.find((f): f is ValueFlag => f.kind === "value" && f.name === name)
}
...
function activeValueFlag(words: string[]): ValueFlag | undefined {
  ...
  if(words[n - 2] === "=") return findValueFlag(words[n - 3])
  if(words[n - 1] === "=") return findValueFlag(words[n - 2])
  // `--flag <value>` — space form; the flag is the previous word.
  return findValueFlag(words[n - 2])
}
...
export function runCompletion(words: string[]): void {
  ...
  if(valueFlag) {
    for(const value of valueFlag.values ?? []) console.log(value)
    return
  }
  ...
  if(current === "" || current.startsWith("-")) {
    for(const flag of FLAGS) console.log(flag.name)
    for(const name of COMPLETABLE_PROVIDER_FLAGS) console.log(name)
  }

  // 3. Directory positional — offer directories unless the user is clearly typing a flag.
  if(!current.startsWith("-")) console.log(DIRS_DIRECTIVE)
}
